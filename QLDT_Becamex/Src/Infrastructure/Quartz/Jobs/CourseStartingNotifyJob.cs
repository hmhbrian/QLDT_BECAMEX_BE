...
namespace QLDT_Becamex.Src.Infrastructure.Quartz.Jobs
{
    ...
    public sealed class CourseStartingNotifyJob : IJob
    {
        ...
        public async Task Execute(IJobExecutionContext context)
        {
            ...
            // 4) Soạn payload
            var (title, body, data) = await _notificationComposer.CourseStartingSoonAsync(courseId, context.CancellationToken);

            var now = ToVietnamTime(DateTime.UtcNow);
            Console.WriteLine($"[Send] CourseStartingNotifyJob for course {courseId} to {tokens.Count} devices at {now}");
