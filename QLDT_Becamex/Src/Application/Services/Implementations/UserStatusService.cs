...
using QLDT_Becamex.Src.Infrastructure.Persistence.UnitOfWork; // Thêm để sử dụng LINQ
    ...
    public class UserStatusService : IUserStatusService
    {
        ...
        public async Task<Result> DeleteAsync(List<int> ids)
        {
            try
            {
                ...
                return Result.Success(message: "Xóa trạng thái người dùng thành công", code: "SUCCESS", statusCode: 200);
            }
            catch (Exception ex)
            {
                return Result.Failure(
