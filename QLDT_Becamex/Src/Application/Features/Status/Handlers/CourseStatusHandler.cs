...
namespace QLDT_Becamex.Src.Application.Features.Status.Handlers
{
    public class CourseStatusHandler :
        IRequestHandler<GetAllCourseStatusesQuery, IEnumerable<CourseStatusDto>>,
        ...
        IRequestHandler<DeleteCourseStatusesCommand, Unit>
    {
        ...
        public async Task<IEnumerable<CourseStatusDto>> Handle(GetAllCourseStatusesQuery request, CancellationToken cancellationToken)
        {
            var list = await _unitOfWork.CourseStatusRepository.GetAllAsync();
            return _mapper.Map<IEnumerable<CourseStatusDto>>(list);
        }
