    ...
    public class SearchCoursesQueryHandler : IRequestHandler<SearchCoursesQuery, PagedResult<CourseDto>>
    {
        ...
        public async Task<PagedResult<CourseDto>> Handle(SearchCoursesQuery request, CancellationToken cancellationToken)
        {
            ...
            if (!string.IsNullOrEmpty(queryParam.DepartmentIds))
            {
                ...
                if (deptIds.Any())
                {
                    Expression<Func<Course, bool>> deptPredicate = c => c.CourseDepartments.Any(cd => deptIds.Contains(cd.DepartmentId));
            ...
            if (!string.IsNullOrEmpty(queryParam.PositionIds))
            {
                ...
                if (posIds.Any())
                {
                    Expression<Func<Course, bool>> posPredicate = c => c.CoursePositions.Any(cp => posIds.Contains(cp.PositionId));
